...
- 合并到 `main` 后，`Build and Release` 会在三个构建全部成功后分别创建或更新 Windows 与 macOS 的 Draft Release，例如 `v2.5.6` 和 `macos-v2.2.4`。
...
Windows 版当前为 2.5.6，macOS 版当前为 2.2.4。两个平台都提供账号隔离的随机心情问候、冷笑话、数学题、趣味知识、脑筋急转弯、生活小贴士和关怀内容互动，并支持 Ed25519 签名内容缓存、离线包、互动设置同步和行为事件批量上传。互动题目与答案会完整换行显示，超长正文可以滚动查看，拖动桌宠时当前互动保持可见。macOS 小剧场包含入场、跳跃、换位和摇摆动作；两端也都支持外观行为、GIF 桌宠管理、资源库、互动词包、提醒、设备绑定和在线更新。Windows 与 macOS 首次使用均可免费试用 10 分钟，之后使用激活码完成绑定；联系作者入口按需展开微信二维码，不会在页面打开时直接展示。macOS 更新会按 Apple Silicon（arm64）和 Intel（x86_64）分别获取对应安装包；Windows 只获取 Windows x64 安装包。两个系统的随机换宠都会使用系统随机源，每轮不重复，并避免轮次交界处连续出现同一张 GIF。
